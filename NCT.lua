...
-- [[ 1. KHỞI TẠO THƯ VIỆN (LIB) & CỬA SỔ CHÍNH ]]
local vu616 = loadstring(game:HttpGet("https://raw.githubusercontent.com/farehamhz/NCTLib/main/NCTLib"))():MakeWindow({
    ["Title"] = "CongTHANYH Hub",
    ["SubTitle"] = "by real_congthanhthanhios",
    ["SaveFolder"] = "CongTHANYH | SKIBITOILET lib v1.lua"
})

-- [[ 2. TẠO ICON NÚT BẤM (CÓ KÉO THẢ & RGB) ]]
...
vu618.Image = "rbxassetid://127353414484118"
...
-- THÊM VIỀN 7 MÀU (RGB) CHO ICON THEO Ý BẠN
...
-- LOGIC KÉO THẢ ICON (GIỮ NGUYÊN TỪ CODE CỦA BẠN)
local vu620 = true
local vu621, vu622, vu623, vu624
...
vu618.InputBegan:Connect(function(pu628)
    if pu628.UserInputType == Enum.UserInputType.Touch or pu628.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        pu628.Changed:Connect(function()
            if pu628.UserInputState == Enum.UserInputState.End then vu621 = false end
        end)
    end
end)

vu618.InputChanged:Connect(function(p629)
    if p629.UserInputType == Enum.UserInputType.Touch or p629.UserInputType == Enum.UserInputType.MouseMovement then vu622 = p629 end
end)

game:GetService("UserInputService").InputChanged:Connect(function(p630)
    if vu621 and p630 == vu622 then vu627(p630) end
end)

-- CLICK ICON ĐỂ ĐÓNG/MỞ MENU
...
-- =========================================================
-- [[ 3. CHÈN LOGIC TỪ b8a8e6363cd8eb26.lua VÀO ĐÂY ]]
-- =========================================================

-- Khử lag và hiệu ứng (Phần đầu file Lua)
...
-- Tạo Tab Info trong Menu của bạn
local InfoTab = vu616:MakeTab({
    Title = "Tab Info",
    Icon = "rbxassetid://127353414484118"
})

-- Thêm chức năng Auto Farm (Ví dụ từ logic file Lua)
InfoTab:AddToggle({
...
-- Nút nhập toàn bộ Code (Lấy danh sách từ cuối file Lua)
local vu1505 = {"KITT_RESET", "Sub2Fer999", "Enyu_is_Pro", "Magicbus", "JCWK", "Starcodeheo", "Bluxxy"}
InfoTab:AddButton({
    Title = "Auto Redeem All Codes",
    Callback = function()
        for _, code in pairs(vu1505) do
            game:GetService("ReplicatedStorage").Remotes.RedeemCode:InvokeServer(code)
        end
    end
})

-- THÊM CÁC HÀM LOGIC QUAN TRỌNG KHÁC (MaterialMon, etc.)
function MaterialMon()
    if _G.SelectMaterial == "Radiactive Material" then
        MMon = "Factory Staff"
        MPos = CFrame.new(-105.88, 72.8, -670.24)
    end
    -- Bạn có thể dán tiếp các hàm khác từ file lua vào đây
end

print("CongTHANYH Hub RGB + Logic Farm Loaded!")
local v631 = vu616
